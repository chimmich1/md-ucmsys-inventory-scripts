...
Current work: **PR-08, 1.0.0 release promotion**.
PR-01 through PR-07 and the PR-08 release-promotion plan are merged. The release
promotion implementation is on `feat/release-1.0.0`.
PR-04 merged as
...
Next action: review the release-promotion pull request, merge it, and create the
1.0.0 tag or GitHub release only as a separate explicit release action.
...
invocation per provider. The real 19-configuration Princess plan probes 25 exact
affected decks instead of 380 blanket probes; a new configuration remains bounded
to 20. Collector failure preserves the active pointer. Historical Princess
catalog entries survive current-voyage absence. A commercial-only change keeps the
physical and category revision IDs and changes only assignment identity.

The revised namespace-independent real snapshot is `02497c227cc9cc7fb008ede8`
under ignored `work/logs/pr05-permanent-masters`; its assessment retains the same
12 targeted items. Runtime state was not migrated and no provider calls were made.

PR-04 verification: 60 tests and read-only legacy Validate pass. Offline assessment
of snapshot `2f9195b160259a2fd5c34edd` produces 12 targeted items: nine missing
required-field groups and three contradictory Princess accessibility observations.
All Celebrity groups and Princess Coral have complete required attributes for their
confirmed cabins. Five Princess groups remain incomplete. All observed assignments
are complete for their source memberships; membership and category definitions
remain unverified. Stalled dimensions create no automatic queue items.
...
| **PR-08** | Promote the accepted collector to 1.0.0 and close the roadmap; no collection behavior changes. | Current-main tests and migrated-state read-only Validate pass; version and release documentation align; no provider calls or runtime migration. | Implementation complete; PR pending |
